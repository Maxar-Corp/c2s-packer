...
func (c *AccessConfig) IsGovCloud() bool {
	...
	if strings.HasPrefix(c.SessionRegion(), "us-gov-") || strings.HasPrefix(c.SessionRegion(), "us-iso-") {
		fmt.Println("++++++++++++++INFO: GovCloud (either us-gov or us-iso detected in region!)")
		return true
	} else {
		fmt.Println("--------------INFO: GovCloud was NOT detected!")
		return false
	}
}
